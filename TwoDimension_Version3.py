...
def specialize_constant_rectangle(width: ti.int32, height: ti.int32, region_x: ti.int32, region_y: ti.int32):  # 矩形特征
    for i, j in ti.ndrange(width, height):
        ...
        mu0_field[ii, jj] = mu_0 * 8
        eps0_field[ii, jj] = eps_0 * 8
...
# 电磁场点状源正弦激励波
@ti.kernel
def electric_pulse_dot_source(t_step_num: ti.int32):  # 反复调用
    real_time = t_step_num*Dt
    pulse = ti.sin(omega * real_time / 5)
    Electric_field[Source_x, Source_y] = Electric_field[Source_x, Source_y] + pulse * 0.4
...
def electric_pulse_rod_source(t_step_num: ti.int32):  # 反复调用
    ...
    pulse = 1.0*ti.cos(omega * real_time / 50)
    # Electric_field[Source_x, Source_y] = Electric_field[Source_x, Source_y] + pulse
    for i in ti.ndrange(20, 10):
        Electric_field[Source_x, Source_y + i - 100] = Electric_field[Source_x, Source_y + i - 100]\
                                                                 + pulse*1.0
...
def main():
    ...
    initialize_constant()
    specialize_constant_rectangle(width=50, height=100, region_x=150, region_y=500)
    specialize_constant_rectangle(width=300, height=50, region_x=275, region_y=575)
    specialize_constant_rectangle(width=300, height=50, region_x=275, region_y=425)
    initialize_field_change()
    while window.running:
        if time_step == 0:
            initialize_mesh_indices()
        electric_pulse_dot_source(t_step_num=time_step)
        ...
        update_all_fields()
        update_vertices()
        camera.position(0.0/f_0*10e9, -15.0/f_0*10e9, 10.0/f_0*10e9)
